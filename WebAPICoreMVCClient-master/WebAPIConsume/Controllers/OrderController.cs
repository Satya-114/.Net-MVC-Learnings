...
namespace WebAPIConsume.Controllers
{
    public class OrderController : Controller
    {
        public async Task<IActionResult> Index()
        {
            ...
            {
                using (var response = await httpClient.GetAsync("https://localhost:44398/api/Order"))
                {
        ...
        public async Task<IActionResult> GetOrder(int id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://localhost:44398/api/Order/" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> AddOrder(Order order)
        {
            if (ModelState.IsValid)
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.PostAsync("https://localhost:44398/api/Order/", content))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
                        order = JsonConvert.DeserializeObject<Order>(apiResponse);
                    }
        ...
        public async Task<IActionResult> UpdateOrder(Order order)
        {
            ...
            if (ModelState.IsValid)
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.PutAsync("https://localhost:44398/api/Order/", content))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> DeleteOrder(int id)
        {
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.DeleteAsync("https://localhost:44398/api/Order/" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                }
            }
